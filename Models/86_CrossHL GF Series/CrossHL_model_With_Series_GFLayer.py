...
class GlobalFilter(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        x = x.view(B, a, b, C)  # a*b==N or (H*W=N in paper)
...
class SingleEncoderBlock(nn.Module):
    ...
    def forward(self, x1,x2):
        ...
        x=self.Globalfilter(x1) # Global Filter after normalizing
        x= self.cross_hl_attention(x,x2) #Cross attention after apply global filters on x1 (HSI)
        x = x + res
        res = x
        x = self.ffn_norm(x)
        x = self.ffn(x)
